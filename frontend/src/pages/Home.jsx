...
const Home = () => {
  ...
  const stats = {
    ...
    criticos: chamados.filter(c => {
      const diff = new Date(c.created_at).getTime() + 15 * 60000 - new Date().getTime();
  ...
  return (
    <div className="min-h-screen p-6 md:p-12 space-y-8 max-w-7xl mx-auto">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-extrabold text-white tracking-tight m-0">
            Gestão de Chamados <span className="text-emerald-500">Massivos</span>
          </h1>
          <p className="text-slate-400 mt-2">Hub operacional inteligente - Service Desk Neoenergia</p>
        </div>
        
        <div className="flex gap-4">
          <div className="bg-slate-800 p-4 rounded-2xl border border-slate-700 text-center min-w-[100px]">
            <span className="block text-slate-500 text-[10px] uppercase font-bold">Total</span>
            <span className="text-2xl font-bold text-white">{stats.total}</span>
          </div>
          <div className="bg-slate-800 p-4 rounded-2xl border border-slate-700 text-center min-w-[100px]">
            <span className="block text-slate-500 text-[10px] uppercase font-bold">Abertos</span>
            <span className="text-2xl font-bold text-emerald-500">{stats.abertos}</span>
          </div>
          <div className="bg-slate-800 p-4 rounded-2xl border border-slate-700 text-center min-w-[100px]">
            <span className="block text-slate-500 text-[10px] uppercase font-bold">Atrasados</span>
            <span className="text-2xl font-bold text-rose-500">{stats.criticos}</span>
          </div>
        </div>
      </header>
      ...
      <main className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-white">📋 Fila de Atendimento</h2>
            <button 
              onClick={() => { if(confirm('Limpar todos os chamados?')) setChamados([]); }}
          ...
          {chamados.length > 0 ? (
            ...
            <div className="bg-slate-800/50 border-2 border-dashed border-slate-700 rounded-3xl p-20 text-center">
              ...
              <p className="text-slate-600 text-sm mt-1">Cole o texto do WhatsApp ao lado para começar.</p>
            </div>
          )}
        </div>
      </main>
      ...
      <footer className="pt-8 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4 text-slate-600 text-xs font-medium">
        ...
        <div>
          Versão MVP 1.0 • Sem dependência de API externa
        </div>
      </footer>
    </div>
  );
