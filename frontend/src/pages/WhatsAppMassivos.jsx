...
const WhatsAppMassivos = () => {
  ...
  const updateStatus = (id, status) => {
    setChamados((prev) => prev.map((c) => (c.id === id ? { ...c, status, updated_at: new Date().toISOString() } : c)));
  };
  ...
  const handleSelectAll = (idsToSelect, isChecked) => {
    if (isChecked) {
  ...
  const { stats, massives } = useMemo(() => {
    ...
    const massives = detectMassiveIncidents(chamados, 3, 30);
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-4">
              <Link to="/" className="px-3 py-1.5 text-xs font-semibold text-slate-300 bg-slate-800/50 border border-slate-700 rounded-lg hover:bg-slate-700 hover:text-white transition-colors flex items-center gap-2 w-max">
            ...
            <p className="text-slate-400 font-medium">Gestão de Chamados Massivos (WhatsApp)</p>
          </div>
        ...
        <main className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={removeAllChamados} className="text-xs text-slate-500 hover:text-rose-400 font-bold uppercase tracking-widest transition-colors">
                Limpar Fila
              </button>
            </div>
